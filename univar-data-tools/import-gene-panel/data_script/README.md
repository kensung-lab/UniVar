# Gene Panels data scripts

## Description
This folder contains scripts for generating [gene panel][gene-panel] [`JSON`][json] files

## Requirements
[Python][python] 3.11

## How to Run
1. Download Clingen disease data [Clingen-Gene-Disease-Summary.csv](https://search.clinicalgenome.org/kb/gene-validity/download) with data/populate.sh
2. Update the version no. and file path in ```config.py```
```python
CLINGEN_GENE_DISEASE_VERSION='2025-09-12' # normally is the date of download
CLINGEN_GENE_DISEASE_PATH='data/Clingen-Gene-Disease-Summary-2025-09-12.csv'
```
3. Run the python script
```bash
python gene_panels.py
```
<!-- 4. Copy the data/ to ../data/ and remove the name start up `panelapp_*.p*.json` -->
