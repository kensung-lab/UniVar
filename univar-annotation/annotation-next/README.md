# Annotation Pipeline with Nextflow
...
This project is using the [Nextflow][nextflow] to create annotation pipeline for [Unified Variant Interpretation Platform][univar].
...
```sh
nextflow run -params-file <yaml_path> annotation.nf
```
...
| ---------------------------- | ------------------------------------------------------------- | ----------------------------------------------------------------------- |
| backend_url                  | [UniVar Backend][univar-backend] url to notify pipeline error | https://{UNIVAR_BACKEND_BASE_URL}                                       |
| nf_secret                    | secure the connection with [UniVar Backend][univar-backend]   | {secure string}                                                         |
| in_snp_file                  | location of the input snp vcf file                            | {proband}\_{trio/duo/solo/n`d`}\_{unique_string}.vcf.gz                 |
| in_sv_files                  | location of the input sv vcf files                            | [{proband}\_{trio/duo/solo/n`d`}\_{unique_string}.{caller}.vcf.gz, ...] |
| data_dir                     | directory of where annotation data is stored                  | /path/to/directory                                                      |
| output_filename              | The expected annotated file name                              | {proband}\_{trio/duo/solo/n`d`}\_{unique_string}                        |
...
| vit_secret                   | secure the connection with [UniVar Backend][univar-backend]   | {secure string}                                                         |
