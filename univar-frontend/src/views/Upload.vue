    ...
    <q-form @submit="onSubmit" class="q-gutter-md">
      <div class="row">
        <div class="col-md-10 required-group">
          <div class="remark-message"><q-icon name="info" />*Required :</div>
          <div class="col-md-10 file-group">
            <q-input
              ...
              label="*Proband ID"
              :rules="[
                (val) => !!val || 'Proband ID is required',
                (val) => val.length <= 50 || 'Please use maximum 50 characters',
                (val) =>
                  /^[a-zA-Z0-9]+$/.test(val) ||
                  'Only English letters (a-z, A-Z) and numbers (0-9) are allowed'
              ]"
            >
              <template v-slot:hint>
                Accepted format: English letters (a-z, A-Z) and numbers (0-9) only
              </template>
            </q-input>
            <br />
              ...
              <q-tab-panels v-model="ped_tab" animated>
                <q-tab-panel name="file">
                  ...
                  >
                    ...
                    <template v-slot:append>
                      <q-icon
                        name="close"
                        @click.stop.prevent="PED_file = null"
                        class="cursor-pointer"
                      />
                    </template>
                    <!-- <template v-slot:hint> * Only accept .ped format </template> -->
                  ...
                  <div class="format-message">
                    <div class="file-format">
                      * Only accept
                      <a
                        href="https://gatk.broadinstitute.org/hc/en-us/articles/360035531972-PED-Pedigree-format"
                        target="_blank"
                        >.ped</a
                      >
                      format
                ...
                <q-tab-panel name="sample">
                  <div class="ped_group_container">
                    <q-btn
                      :loading="getPEDInfoLoading"
                      square
                      push
                      color="primary"
                      label="Get PED information from VCFs"
                      @click="getPEDInfoFromVCF()"
                    />
                    <div class="row each_ped_group" v-for="(row, index) in ped_group" :key="index">
                      <div class="col-md-3">
                        <q-input v-model="row.sample_id" label="Sample ID" />
                      </div>
                      <div class="col-md-2">
                        <q-select v-model="row.sex" :options="gender_options" label="Gender" />
                      </div>
                      <div class="col-md-2">
                        <q-select
                          v-model="row.affected"
        ...
        <div class="col-md-10 optional-group">
          <div class="remark-message"><q-icon name="info" />Upload at least one of them :</div>
          <div class="col-md-10 file-group">
            <q-file
              outlined
              bottom-slots
            ...
            <div class="format-message">
              <div class="file-format">
                Only accept:
                <a href="https://samtools.github.io/hts-specs/VCFv4.5.pdf" target="_blank"
                  >.vcf.gz</a
                >
                format
              </div>
          ...
          <div class="col-md-10 file-group">
            ...
            <template v-if="svCallers && svCallers.length > 0">
              <div class="row" v-for="(row, index) in svCallers" :key="index">
                <div class="col-md-3">File Name : {{ row.filename }}</div>
                <div class="col-md-4">
                  <q-input
                    v-model="row.caller"
                    bg-color="#e0e0e0"
  ...
  watch: {
    ...
    SV_files: function (newVal) {
      if (!this.svCallers) {
        ...
        const extractCaller = (filename) => {
          if (!filename) return ''
          const nameWithoutExt = filename.replace(/\.vcf\.gz$/, '')
          const parts = nameWithoutExt.split('.')
          return parts.length > 0 ? parts[parts.length - 1] : nameWithoutExt
        }
        // remove deleted
        if (newVal === null) {
          this.svCallers = []
        } else {
          this.svCallers = this.svCallers.filter((callers) =>
            newVal.some((file) => callers.filename === file.name)
          )
          newVal.forEach((file) => {
            if (!this.svCallers.some((callers) => file.name === callers.filename)) {
              this.svCallers.push({
                filename: file.name,
                caller: extractCaller(file.name)
              })
            }
          })
        }
    ...
    async onSubmit(evt) {
      ...
      try {
        uploadResult = await store.getters.getApiService.uploadPipelineFile(
          ...
          this.proband_id,
          this.ped_group
            .filter((ped) => ped.sample_id !== '' && ped.sex !== '' && ped.affected !== '')
            .map((ped) => {
              return {
